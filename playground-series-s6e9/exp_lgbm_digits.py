#!/usr/bin/env python3
"""S6E9 exp: TE recipe + income ones/tens/hundreds digits as categoricals."""
...
def add_static(df: pd.DataFrame) -> pd.DataFrame:
    ...
    out["income_ones"] = (inc % 10).astype(str)
    out["income_tens"] = (inc // 10 % 10).astype(str)
    out["income_hundreds"] = (inc // 100 % 10).astype(str)
    return out
...
def main() -> None:
    ...
    print("model TE LGBM + income ones/tens/hundreds digits as cats")
    ...
    np.save(DATA / "oof_lgbm_digits.npy", oof)
    out = DATA / "submission_lgbm_digits.csv"
