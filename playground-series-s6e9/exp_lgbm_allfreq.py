#!/usr/bin/env python3
"""S6E9 exp: TE recipe + unsupervised train+test frequency of every static column."""
...
NUMERIC = [
    ...
    "income_te",
    "commute_te",
] + [f"{c}_allfreq" for c in FREQ_SRC]
CATEGORICAL = [
    "Gender",
    "City_Type",
    "Current_Car_Type",
...
def add_allfreq(train_s: pd.DataFrame, test_s: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame]:
    both = pd.concat([train_s[FREQ_SRC], test_s[FREQ_SRC]], ignore_index=True)
    maps = {c: both[c].value_counts(normalize=True) for c in FREQ_SRC}
    tr = train_s.copy()
    te = test_s.copy()
    for c, m in maps.items():
        tr[f"{c}_allfreq"] = tr[c].map(m).astype(np.float64)
        te[f"{c}_allfreq"] = te[c].map(m).astype(np.float64)
    return tr, te
...
def main() -> None:
    ...
    train_s, test_s = add_allfreq(add_static(train), add_static(test))
    cols = NUMERIC + CATEGORICAL

    print("train", train.shape, "test", test.shape)
    print("model TE LGBM + train+test frequency of every static column")
    ...
    print(f"baseline lgbm_te oof={BASELINE_OOF:.5f}")
    print(f"delta_oof={delta:+.5f} decision={decision}")

    np.save(DATA / "oof_lgbm_allfreq.npy", oof)
    out = DATA / "submission_lgbm_allfreq.csv"
