#!/usr/bin/env python3
"""S6E9 exp: TE recipe + original survey rows in training folds only."""
...
def main() -> None:
    ...
    y_orig = encode_target(orig[TARGET])
    train_s = add_static(train)
    orig_s = add_static(orig)
    test_s = add_static(test)
    cols = NUMERIC + CATEGORICAL

    print("train", train.shape, "orig", orig.shape, "test", test.shape)
    print("model TE LGBM + original survey rows in train folds only")
    ...
    for fold, (tr, va) in enumerate(cv.split(train_s, y), start=1):
        src = pd.concat([train_s.iloc[tr], orig_s], ignore_index=True)
        y_src = np.concatenate([y[tr], y_orig])
    ...
    np.save(DATA / "oof_lgbm_orig.npy", oof)
    out = DATA / "submission_lgbm_orig.csv"
