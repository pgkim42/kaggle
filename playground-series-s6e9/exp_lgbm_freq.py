#!/usr/bin/env python3
"""S6E9 exp: slow LGBM + fold-safe income/commute frequencies (synthetic floors)."""
...
def add_fold_freq(base: pd.DataFrame, src: pd.DataFrame) -> pd.DataFrame:
    out = base.copy()
...
def main() -> None:
    ...
    print("model slow LGBM + fold-safe income/commute freq + floor flags")
    ...
    for fold, (tr, va) in enumerate(cv.split(train_s, y), start=1):
        src = train_s.iloc[tr]
        X_tr = add_fold_freq(src, src)[cols]
        X_va = add_fold_freq(train_s.iloc[va], src)[cols]
        X_te = add_fold_freq(test_s, src)[cols]
    ...
    out = DATA / "submission_lgbm_freq.csv"
